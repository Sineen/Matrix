...
const string NEGATIVE_NUMBER = "you gave a negative number or rows/ columns";
...
public:
     ...
     * throws exeption if rows or colums are negative
     */
     ...
     * throws exeption if number is negative
     */
     ...
     * throws an exception in case the sizes are not equal  aka number of rows not equale or number of columns
     *
     */
    Matrix<T> operator+(const Matrix &other);
...
Matrix<T>::Matrix(unsigned int rows, unsigned int cols)
{
    ...
    {
        throw Exceptions(NEGATIVE_NUMBER);
    }
    this->size = rows*cols;
